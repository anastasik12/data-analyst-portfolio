...
- Написаны запросы с оконными функциями, JOIN, подзапросами:  
  ...
  - дата предыдущего заказа для каждого клиента (`LAG()`);
  - скользящее среднее суммы заказа за последние 3 дня (`AVG OVER`).

**Файлы:**
- [Схема БД](sql_shop_database/schema.sql)
- [ER-диаграмма (физическая модель)](sql_shop_database/physical_diagram.png)
